...
export function ProfileCards({ onSelectProfile }: ProfileCardsProps) {
    return (
        <section className="shrink-0 py-4 px-4 max-w-6xl mx-auto w-full">
            <motion.div
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {profiles.map((profile, index) => (
                    ...
                    >
                        <Card
                            variant="default"
                            className={`h-full border-l-4 ${profile.colorClass} cursor-pointer hover:bg-bg-hover transition-all duration-150 relative flex flex-col gap-2 p-4`}
                        >
                            <profile.Icon
                                size={18}
                            ...
                            <div>
                                <h3 className="text-base font-bold text-text-primary font-display leading-tight mb-1">
                                    {profile.title}
                                </h3>
